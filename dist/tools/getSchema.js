 ...
 * 스키마 정보 조회 핸들러
 */
async function getSchemaHandler(params) {
    try {
        // 입력 검증
 ...
 * 모든 테이블 목록을 조회합니다
 */
async function getAllTables(dbManager, dbPath) {
    try {
    ...
    `;
        ...
        const tables = queryResult.data?.map(row => row.name) || [];
        return {
            success: true,
            tables
        };
 ...
 */
async function getTableSchema(dbManager, dbPath, tableName) {
    try {
        // 테이블 존재 여부 확인

 ...
 * 메타 명령어 도구 정의
 */
export const metaCommandTool: ToolDefinition = {
  name: 'meta_command',
 ...
 * 메타 명령어 핸들러
 */
async function metaCommandHandler(params: MetaCommandInput): Promise<ToolResult> {
 ...
 * .tables 명령어 처리
 */
async function handleTablesCommand(dbManager: DatabaseManager, dbPath: string): Promise<MetaCommandResult> {
  const query = `
    SELECT name, type
    FROM sqlite_master
 ...
 * .schema 명령어 처리
 */
async function handleSchemaCommand(dbManager: DatabaseManager, dbPath: string, target?: string): Promise<MetaCommandResult> {
  let query: string;
  let params: any[] = [];
 ...
 * .indexes 명령어 처리
 */
async function handleIndexesCommand(dbManager: DatabaseManager, dbPath: string, target?: string): Promise<MetaCommandResult> {
  let query: string;
  let params: any[] = [];
 ...
 * .pragma 명령어 처리
 */
async function handlePragmaCommand(dbManager: DatabaseManager, dbPath: string, target?: string): Promise<MetaCommandResult> {
  if (!target) {
    return {
      success: false,
      result: '',
